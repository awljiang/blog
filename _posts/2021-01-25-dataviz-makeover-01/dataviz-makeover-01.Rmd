...
description: |
  This is the first DataViz Makeover for SMU ISSS608 Visual Analytics and Applications course.
  
  Objective: To create an alternative visualisation to understand Singapore's labour force in 2019. 
  
  Original visualisation can be found in chart 6 of the report "Labour Force in Singapore 2019".
  
  (https://stats.mom.gov.sg/Pages/Labour-Force-Tables2019.aspx) 
author:
  - name: Angeline Jiang
...
Rename the columns to include “(old)”
...
```
Step 5: Right click on the drop down list under "Age" column, click Create group to 3 groups "15 - 24", "25 - 54" and "55 & over" by highlighting the subgroups and click on the group button. Rename the group and repeat for the other 2 groups.
...
Step 1: Download Labour Force Participation rate dataset from Singstat Table Builder (https://www.tablebuilder.singstat.gov.sg/publicfacing/mainMenu.action) and follow the illustration below
...
Delete rows 1-5, 7, 19 onwards, rename column A to “Age”. Save the Excel file.
...
Step 4: Right click on the drop down list under "Age" column, click Create group to 3 groups "15 - 24", "25 - 54" and "55 & over" by highlighting the subgroups and click on the group button. Rename the group and repeat for the other 2 groups.
...
Step 7: Drag the csv file saved in Step 6 into the pane, right click the drop down list for the column "Year_In/Out" and click on Split. Rename the new columns "Year" and "In/Out".
...
Step 1: Download "Table (2) Key Characteristics of Resident Labour Force, 2009 – 2019 (June)" from https://stats.mom.gov.sg/Pages/Labour-Force-Tables2019.aspx. Table as shown below
...
Step 1: Import csv file for resident labour force shares saved in 3.1.1 (step 8).
...
Step 5: Drag in the participation rate file to create a union with the shares file and edit the relationship as shown below.
...
##### Do steps 1 - 5 to create visualisation for 2019 labour force data, and steps 6 - 8 onwards for comparison with 2009 and 2014.
...
Step 8: Open a new Tableau worksheet. Drag the variables into "Columns" and "Rows" as shown below. Drag "Percentage Change wrt 2009" and "Percentage Change wrt 2014" into their respective text marks card for the label to appear and format the label to show in "pp" (similar to step 2). Click on the x-axis (click on the x-axis, then format to ensure that the range for both charts are the same.) and y-axis and un-tick "Show Header" to hide the axis. Drag the respective dimension into colour marks card to format the colour of the bar and format the colour of the bar as shown below. 
...
https://public.tableau.com/profile/angeline.jiang#!/vizhome/DataVizMakeover_01_v2/FinalDashboard

```{r, echo=FALSE, message=FALSE, layout="l-body-outset", out.width="120%", out.height="180%"}
library(knitr)
library(png)
img4_path <- "/awljiang/blog/_posts/2021-01-25-dataviz-makeover-01/data/4.2_final design.png"
